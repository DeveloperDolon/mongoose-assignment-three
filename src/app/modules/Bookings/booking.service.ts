...
const getUserBookingsFromDB = async (userInfo: JwtPayload) => {
  const user = await User.findOne({ email: userInfo?.userEmail }, { _id: 1 });

  const bookings = await Booking.find({ customer: user?._id }, { customer: 0 })
    .populate('serviceId')
    .populate('slotId');
